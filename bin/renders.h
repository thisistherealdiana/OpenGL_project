...
int drow_bilbords( GLuint sh , GLuint vao, GLuint btexture, glm::mat4 view, glm::mat4 projection, glm::vec3 camera_pos ){
  ...
  glm::vec3(3.2f,  0.5f,  6.0f), glm::vec3(3.9f,  0.5f,  7.0f) };
    for(int i = 0; i < 6; i++){
        bilboard_position[i] = bilboard_position[i] + glm::vec3(5.0f, 1.0f, -4.0f);
    }
...
GLfloat plane_vertices_mainOLD[] = {
///////////////////////////////////////
...
GLuint set_cubes_vao1(){
    GLuint vbo, vao;
    glGenBuffers(1, &vbo);
    glGenVertexArrays(1, &vao);
    glBindVertexArray(vao);
    glBindBuffer(GL_ARRAY_BUFFER, vbo);
    glBufferData(GL_ARRAY_BUFFER, sizeof(vertices_for_cube_with_nornals), vertices_for_cube_with_nornals, GL_STATIC_DRAW); 
    glEnableVertexAttribArray(0); 
    glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 11 * sizeof(GLfloat), (GLvoid*)0);
    glEnableVertexAttribArray(1); 
    glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, 11 * sizeof(GLfloat), (GLvoid*)(3*sizeof(GLfloat)));
...
public:
    ...
    float shin;
    GLuint normals;
    glm::mat4 matrix_for_shadow;
    GLuint texture_for_shadow;
    GLuint paralax_m;
    bool paralax;
};
...
int load_textures(Material &material){
    ...
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_LINEAR);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR); 
    //image = stbi_load("./textures/Tiles_03/Tiles_03_basecolor.jpg", &tex_width, &tex_height, &n, 0);
    image = stbi_load("./textures/Tiles_04/Tiles_04_basecolor.jpg", &tex_width, &tex_height, &n, 0);
    if(image == NULL)
    {
...
material.diffuse = new_texture;
    ...
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_LINEAR);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR); 
    //image = stbi_load("./textures/Tiles_03/Tiles_03_glossiness.jpg", &tex_width, &tex_height, &n, 0);
    image = stbi_load("./textures/Tiles_04/Tiles_04_glossiness.jpg", &tex_width, &tex_height, &n, 0);
    if(image == NULL)
    {
...
material.specular = new_texture;
    ...
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_LINEAR);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR); 
    //image = stbi_load("./textures/Tiles_03/Tiles_03_normalOgl.jpg", &tex_width, &tex_height, &n, 0);
    image = stbi_load("./textures/Tiles_04/Tiles_04_normalOgl.jpg", &tex_width, &tex_height, &n, 0);
    if(image == NULL)
    {
...
material.normals = new_texture;
    ...
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_LINEAR);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR); 
    //image = stbi_load("./textures/Tiles_03/Tiles_03_height.jpg", &tex_width, &tex_height, &n, 0);
    image = stbi_load("./textures/Tiles_04/Tiles_04_height.jpg", &tex_width, &tex_height, &n, 0);
    if(image == NULL)
    {
        std::cout << "error open height texture " << std::endl;
        return  1;
    }
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, tex_width, tex_height, 0,  GL_RGB, GL_UNSIGNED_BYTE, image);
    glGenerateMipmap(GL_TEXTURE_2D);
    glBindTexture(GL_TEXTURE_2D, 0);
    stbi_image_free(image);
material.paralax_m = new_texture;
    return 0;
}

int drow_plane1(GLuint shad, GLuint vao, int count, Material material_l, glm::vec3 camera_pos, glm::vec3 light_pos,  glm::mat4 model,  glm::mat4 view, glm::mat4 projection, 
    bool tr_eff = false){
        glUseProgram(shad);
        ...
        glUniformMatrix4fv(glGetUniformLocation(shad, "shadowmat"), 1, GL_FALSE, glm::value_ptr(material_l.matrix_for_shadow));
        ...
        glUniform1i(glGetUniformLocation(shad, "for_light.tr_eff"), tr_eff);   
        glUniform1i(glGetUniformLocation(shad, "for_light.paralax"), material_l.paralax);   
        //std::cout << material_l.paralax << std::endl;
        ...
        glBindTexture(GL_TEXTURE_2D, material_l.normals);
        glUniform1i(glGetUniformLocation(shad, "material.normalm"), 2);

        glActiveTexture(GL_TEXTURE3);
        glBindTexture(GL_TEXTURE_2D, material_l.texture_for_shadow);
        glUniform1i(glGetUniformLocation(shad, "material.shadow"), 3);

        glActiveTexture(GL_TEXTURE4);
        glBindTexture(GL_TEXTURE_2D, material_l.paralax_m);
        glUniform1i(glGetUniformLocation(shad, "material.height_map"), 4);
...
int shadow_buffer(GLuint &buffer, GLuint &texture, int sh_w, int sh_h){
    glGenFramebuffers(1, &buffer);
    glGenTextures(1, &texture);
    glBindTexture(GL_TEXTURE_2D, texture);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_DEPTH_COMPONENT,  sh_w, sh_h, 0, GL_DEPTH_COMPONENT, GL_FLOAT, NULL);
